        ...
        {filteredGroupedUsers.map(([userId, userData]) => (
          <div key={userId} className="user-card" onClick={() => handleUserCardClick(userId)}>
            {userData.profileImage ? (
              <img src={`http://localhost:8000${userData.profileImage}`} alt={userData.username} className="user-avatar" />
            ) : (
          ...
          <div className="modal-box" ref={modalRef}>
            <div className="user-info">
              {selectedUserData.profileImage ? (
                <img src={`http://localhost:8000${selectedUserData.profileImage}`} alt="User" className="user-avatar" />
              ) : (
                <div className="user-avatar placeholder">
